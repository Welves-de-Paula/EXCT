    ...
    for nome_arquivo in files:
        if nome_arquivo.lower().endswith('.xml'):
            ...
            try:
                ...
                # Tenta localizar <emit> com e sem namespace
                emit = root_xml.find('.//nfe:emit', ns)
                if emit is None:
                    emit = root_xml.find('.//emit')
                ...
                if emit is not None:
                    ...
                    cpf_tag = emit.find('nfe:CPF', ns)
                    if cpf_tag is None:
                        cpf_tag = emit.find('CPF')
                ...
                pasta_emitino = os.path.join(pasta_raiz, identificador)
                os.makedirs(pasta_emitino, exist_ok=True)

                novo_caminho = os.path.join(pasta_emitino, nome_arquivo)

                # Evita sobrescrever
                i = 1
                base_nome, ext = os.path.splitext(nome_arquivo)
                while os.path.exists(novo_caminho):
                    novo_caminho = os.path.join(pasta_emitino, f"{base_nome}_{i}{ext}")
                    i += 1
